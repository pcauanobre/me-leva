...
interface Props {
  ...
  statusFilter: string;
  forms: AdoptionFormRow[];
  adoptedList: AdoptedAnimal[];
  linkedForms: LinkedForm[];
}

export default function AdocaoClient({
  tab,
  statusFilter,
  forms,
  adoptedList,
  ...
  const formByAnimal = new Map<string, LinkedForm>(
    linkedForms.filter((f) => !!f.animal_id).map((f) => [f.animal_id, f]),
  );
    ...
    <Box>
      ...
      <Paper sx={{ mb: 3 }}>
        <Tabs
          ...
          scrollButtons="auto"
          onChange={(_, newValue) => {
            const target = mainTabs.find((t) => t.value === newValue);
        ...
        <Box>
          <Paper sx={{ mb: 3 }}>
            <Tabs
              ...
              sx={{ minHeight: 40 }}
              onChange={(_, newValue) => {
                const target = statusTabs.find((t) => t.value === newValue);
                if (target) router.push(target.href);
              }}
            >
              {statusTabs.map((t) => (
